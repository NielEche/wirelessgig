    ...
    <div class="container-fluid mx-auto pt-16  bg-SelectColor">
        ...
        <div class=" flex flex-wrap">
            @foreach($blogs as $blog)
                    ...
                    <div class="relative space-y-1 pt-4 border-b  border-black dark:border-black">
                        <div class=" text-center">
                            ...
                            <div class="flex justify-between border-t  border-black dark:border-black py-4">
                                <a x-data="" x-on:click.prevent="$dispatch('open-modal', '{{ $blog->id }}')" class="text-sm underline  text-black">EDIT</a>   
                              
                                <a x-data="" x-on:click.prevent="$dispatch('open-modal', 'delete{{ $blog->id }}')"
                                    class="text-sm DINAlternateBold">Delete</a>
                            </div>
                            <!--<div class="flex justify-between border-t  border-black dark:border-black py-4">
                               <a href="{{ route('event.speakers', $blog->id) }}"
                                    class="text-sm DINAlternateBold">Add Speakers</a>
                                <a href="{{ route('event.schedule', $blog->id) }}"
                                    class="text-sm DINAlternateBold">Add Schedule</a>
                            </div>-->
                        </div>
                    </div>
                </div>
                <x-modal name="{{ $blog->id }}" focusable>
                    <!-- Modal Content -->
                    <form method="post"
                        ...
                        <div class="mt-6 form-check">
                            <input type="checkbox" class="form-check-input" id="publishCheckbox" name="publish" {{ $blog->publish ? 'checked' : '' }}>
                        ...
                        <div class="mt-6 ">
                            ...
                            <x-text-input id="date" name="date" type="text" class="mt-1 block w-full"
                                :value="old('date', $blog->date)" required autofocus autocomplete="date" />
                        ...
                        <div class="mt-6 ">
                            ...
                            <x-text-input id="theme" name="theme" type="text" class="mt-1 block w-full"
                                :value="old('theme', $blog->theme)" required autofocus autocomplete="theme" />
                        ...
                        <div class="mt-6 ">
                            <p class="orpheusproMedium" for="about" :value="__('about')">Content</p>
                            <textarea id="about" name="about"
                                ...
                                required autofocus
                                autocomplete="about">{{ old('about', $blog->about) }}</textarea>
                ...
                <x-modal name="delete{{ $blog->id }}" focusable>
                    <!-- Modal Content -->
                    <form method="post"
                        action="{{ route('blogDelete.destroy', $blog->id) }}"
       ...
       <x-modal name="addEvent" focusable>
            <!-- Modal Content -->
            <form method="post" action="{{ route('blog.store') }}" class="p-6 bg-emerald-500"
